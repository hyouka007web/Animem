...
export default async function TagDetailPage({ params }: { params: { slug: string } }) {
  ...
  const [series, movies] = await Promise.all([
    findByTaxonomy(pb, COL.series, "tag_ids", tag!.id),
    findByTaxonomy(pb, COL.movies, "tag_ids", tag!.id),
  ]);

  return (
    <div className="min-h-screen bg-neutral-950 px-4 py-8 text-white">
      <div className="mx-auto max-w-6xl">
        <h1 className="mb-6 text-2xl font-bold">Tag: #{tag!.name}</h1>
        ...
        {!series.length && !movies.length && (
          <p className="text-sm text-neutral-500">Noch nichts mit diesem Tag.</p>
        )}
      </div>
    </div>
  );
