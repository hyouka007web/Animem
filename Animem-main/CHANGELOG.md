# 2026-08-26 – Security & Production Hardening

- Fremden Android/APK-GitHub-Workflow entfernt.
- CI auf npm + Node 20 standardisiert und TypeScript-Check ergänzt.
- Sicherheitsheader über Middleware ergänzt.
- Remote-Image-Quellen auf HTTPS-Host-Allowlist umgestellt.
- Zod-Validierung für zentrale API-Eingaben vereinheitlicht.
- Rate-Limits für Auth, Forum, Uploads, Tickets, Ratings und Benutzeraktionen ergänzt.
- Gebannte Benutzer serverseitig aus Sessions ausgeschlossen.
- Uploads auf serverseitigen `/api/uploads`-Endpunkt verschoben und auf 10 MB sowie PNG/JPEG/WebP/GIF begrenzt.
- Watchlist, Abos, Profil-Favoriten und Ratings gegen doppelte Einträge abgesichert.
- Serien-/Staffel-/Episoden-Beziehungen bei Änderungen verifiziert.
- Serien-/Movie-Löschvorgänge räumen abhängige Daten auf.
- Forum- und Ticket-Listen paginiert und Profil-/Target-Enrichment gebündelt.
- `ARCHIVED` für Serienstatus korrekt unterstützt.
- Render-Konfiguration und aktuelle Betriebsanleitung ergänzt.
